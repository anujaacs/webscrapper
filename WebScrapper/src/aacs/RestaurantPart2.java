...
public class RestaurantPart2 {
	...
	public static void main(String[] args) throws JSONException {
		...
		try {
			...
			fh = new FileHandler("appender2.log");
		...
		driver.quit();
		try {
			FileWriter fileWriter = new FileWriter("first26resArray.json");
	...
	public void comments() {
		/*
		 * [{"lid":"C1L1", "lrestaurants":[{"resName": "XYZ", "resUrl": "xyz.com",
		 * "resRatings": "4.4", "resCost":"1200"}]}]
		 * driver.get(loaclityElement.attr("href") + "?all=1"); Document restaurantsDoc
		 * = Jsoup.parse(driver.getPageSource()); Element pagesTotal =
		 * restaurantsDoc.select("div.pagination-number div b").last(); int totalPages =
		 * Integer.parseInt(pagesTotal.html()); JSONArray restaurantsArray = new
		 * JSONArray(); for (int i = 0; i < totalPages; i++) { Elements resNames =
		 * restaurantsDoc.select("a.result-title"); Elements resRatings =
		 * restaurantsDoc.select("div.rating-popup"); Elements resAddresses =
		 * restaurantsDoc.select("div.search-result-address"); Elements resCosts =
		 * restaurantsDoc.select("div.res-cost span.pl0"); //resNames.get(1).html(); int
		 * namesSize = resNames.size(); for (int a = 0; a < namesSize; a++) { JSONObject
		 * restaurantObj = new JSONObject(); String resId = localityId + "R" + (a+1);
		 * try { restaurantObj.put("resName", resNames.get(a).html());
		 * restaurantObj.put("resUrl", resNames.get(a).attr("href")); if (a <
		 * resRatings.size()) restaurantObj.put("resRating", resRatings.get(a).html());
		 * else restaurantObj.put("resRating", "NA"); if (a < resAddresses.size())
		 * restaurantObj.put("resAddress", resAddresses.get(a).html()); else
		 * restaurantObj.put("resAddress", "NA"); if (a < resCosts.size())
		 * restaurantObj.put("resCostForTwo", resCosts.get(a).html()); else
		 * restaurantObj.put("resCostForTwo", "NA"); restaurantObj.put("resId", resId);
		 * } catch (JSONException je) {
		 * System.out.println("[ERROR RESTAURANT]JSON exception ocuured " +
		 * je.getMessage() +"\n "+ resNames.get(a).attr("href")+"\n PAGE No"+(i+1));
		 * screenshotUrlAdd(driver, "ERRORRESTAURANT.png"); }
		 * driver.get(resNames.get(a).attr("href")); restaurantsDoc =
		 * Jsoup.parse(driver.getPageSource()); for (Element menuPage:
		 * restaurantsDoc.select("script")) { for (DataNode node: menuPage.dataNodes())
		 * { if (node.getWholeData().contains("zomato.menuPages")) {
		 * //System.out.println(node.getWholeData()); String pages =
		 * node.getWholeData().split("zomato.menuPages = ")[1]; try { JSONArray
		 * menuArray = new JSONArray(pages.split(";")[0]);
		 * restaurantObj.put("resMenuPages", menuArray); }catch (JSONException je) {
		 * System.out.println("[ERROR MENU PAGE]JSON exception ocuured " +
		 * je.getMessage() +"\n "+ resNames.get(a).attr("href")+"\n PAGE No"+(i+1));
		 * screenshotUrlAdd(driver, "ERRORMENUPAGE.png"); } } } }
		 * restaurantsArray.put(restaurantObj); }
		 * 
		 * //System.out.println(names.html()); driver.get(loaclityElement.attr("href") +
		 * "?all=1&page=" + (i + 1)); restaurantsDoc =
